...
export const Capsule = () => {
    ...
    return (
        <div className={`border-[rgba(255,255,255,0.2)] absolute top-[15%] transition-all duration-[0.5s] border py-2 px-4 rounded-3xl bg-[rgba(36,36,36)] shadow-md ${typingContext.isTyping ? 'opacity-0' : 'opacity-100'} font-['Josefin_Sans'] flex gap-4`}>
            ...
            <div
                ...
                onClick={() => {
                    typingContext.complete && typingContext.setComplete(false);
                    typingContext.getNewPara();
                    typingContext.setTimeRemaining(typingContext.setting.duration)
                    typingContext.setIsRunning(false)
                }}
            >
                {typingContext.complete ? "Retry" : "Update"}
            </div>
        </div>
    )
}
